...
public class Controller {
    ...
    * Inserting the "SLEEP" input will trigger the last thread to create a short delay in addition. */
    public synchronized void barrier(int action) {
        finished++;
        if (finished >= maxThreads) {  // Only last thread will succeed in getting here, and will not be asked to wait
            finished=0;
            ...
            notifyAll();// last thread will get here only after updating the original board from tempBoard.
        }
        else{
